...
import com.gome.stage.item.*;
import com.gome.stage.service.*;
 ...
 * 	    4：因为旧数据格式无规律，只能舍弃部分数据。从任何数据库取值是，只取site = ‘gome’
 * 	详细逻辑参看下文注释
 */
@Controller
@RequestMapping(value = "/topicnew")
public class SubjectController {
    ...
    /*	@Autowired
        private IHotSearchwordService hotSearchwordService;*/
    @Autowired
    ...
    public ModelAndView getproductWordsList(HttpServletRequest request,HttpServletResponse response) {
        ...
        ModelAndView modelAndView = new ModelAndView("vm-subjectTitle");
     ...
     *
     * 	     因为调用接口多，导致线上响应过慢，所以部分块中的数据已通过定时任务存入redis，会先从redis取数据
     */
    ...
    public ModelAndView getTitleIndexOrTitleData(@PathVariable(value = "keyWord",required = true) String keyWord,HttpServletRequest request,HttpServletResponse response) {
        ...
        if (isInteger(keyWord)){
            if (keyWord.length()==5){
                ...
                ModelAndView modelAndView = new ModelAndView("vm-productSummarize");

                List<Map<String, Object>> productComments = null;
                List<Map<String, Object>> productAdvisory = null;
                if (cache.exists("seotopic_commentsZongShu/"+titleId)){
                    try {
                        String productCommentsString = cache.get("seotopic_commentsZongShu/"+titleId);
                        ...
                        cache.set("seotopic_commentsZongShu/"+titleId,commentsZongShuString);
                    }
                }else {
                    productComments = getProductComments(Integer.valueOf(titleId), "zongshu_product_comment");
                    String commentsZongShuString = JSON.toJSONString(productComments);
            ...
            if (keyWord.length()>=6){
                ...
                if (host.contains("m.gome.com.cn")){
                    modelAndView = new ModelAndView("vm_topic_wap/vm-productKeyWords");
                }else {
                    modelAndView = new ModelAndView("vm-productKeyWords");
                }

                String realKeyword = keyWord.replaceAll("[^(0-9)]", "");
                if (realKeyword.length()!=6){
                    if (host.contains("m.gome.com.cn")){
                ...
                Keywords tag = keyWordService.getTagnameById(Integer.valueOf(realKeyword));
                if(tag!=null){
                    modelAndView.addObject(TAGKEY, tag);
                    List<Title> titleList = titleService.findZhuanTiByTagId(Integer.valueOf(realKeyword));
                    modelAndView.addObject("title_list", titleList);
                }else {
                    if (host.contains("m.gome.com.cn")){
                        return new ModelAndView("redirect301:/mtopic/");
                    }else {
        ...
        if (keyWord.length()<=3){
            ...
            if (cindexList.contains(firstLetter)){
                ModelAndView modelAndView = new ModelAndView("vm-subjectTitlePage");
                List<Map<String,Object>> cindexCountList = titleService.getTitleCindexCount(SITE);
                if(null != cindexCountList) {
                    if (null != cindexList) {
                        Map<String, Object> keyMap = cindexCountList.get(0);
                        for (String key : cindexList) {
                            if (!keyMap.containsKey(key)) {
                                keyMap.put(key, 1L);
                            }
                        }
                    }
                    modelAndView.addObject(CINDEX_COUNTS, cindexCountList);
                }
                modelAndView.addObject(CINDEX_KEY,cindexList);
                modelAndView.addObject(CINDEX,keyWord);
                if (keyWord.contains("-")){
                    String pageNum = keyWord.substring(keyWord.indexOf("-")+1);
                    String realKeyWord = keyWord.substring(0,keyWord.indexOf("-"));
                    try{
                        // 获得数据分类以及总条数
                        cindexKeyList = new HashMap<String, Object>();
                        cindexKeyList.put("cindex_key", cindexList);
                        Map<String, Object> cindexMap = cindexCountList.get(0);
                        if (null != cindexMap && !cindexMap.isEmpty()){
                            Long counts = (Long) cindexMap.get(realKeyWord);// 获得索引对应的数量
                            int pages = (int) Math.ceil(counts * 1.0 / pageSize);

                            // 获得页码数，同时获得每一页对应的数据
                            Map<String, Object> search = new HashMap<String, Object>();
                            search.put("pageNumber", Integer.valueOf(pageNum));
                            search.put("totalCount", pages);
                            search.put("totalCount2", counts.intValue());
                            search.put("pageSize", pageSize);
                            search.put("site", SITE);
                            if ("0".equals(realKeyWord))
                            {
                                search.put("cstart", "0");
                                search.put("cend", "9");
                            }
                            ...
                            {
                                search.put("cindex", realKeyWord);
                            }
                            Map<String, Object> cdata = titleService.getTitleMapWithPage(search);
                            // 计算分页;
                            Map<String, Object> pageMap = processPageNum(search);
                            cdata.putAll(pageMap);
                            search.put("cindex", realKeyWord);// 查询为零的时候添加
                            cdata.putAll(search);
                            cdata.put("withPages", "yes");// 是否分页
                            cdata.putAll(cindexKeyList);// 添加 cindex_key
                            modelAndView.addAllObjects(cdata);
                        }
                    }catch (Exception e){
                        logger.error(e.getMessage(), e);
                        logger.info("ZhuantiTitleListPagePublishTask_pusblis_error:" + e.getMessage());
                    }
                    return modelAndView;
                }else{
                    try{
                        ...
                        Map<String, Object> cindexMap = cindexCountList.get(0);
                        if (null != cindexMap && !cindexMap.isEmpty()){
                            Long counts = (Long) cindexMap.get(keyWord);// 获得索引对应的数量
                            int pages = (int) Math.ceil(counts * 1.0 / pageSize);
                            // 获得页码数，同时获得每一页对应的数据
                            Map<String, Object> search = new HashMap<String, Object>();
                            search.put("pageNumber", 1);
                            search.put("totalCount", pages);
                            search.put("totalCount2", counts.intValue());
                            search.put("pageSize", pageSize);
                            search.put("site", SITE);
                            ...
                            Map<String, Object> pageMap = processPageNum(search);
                            cdata.putAll(pageMap);
                            search.put("cindex", keyWord);// 查询为零的时候添加
                            cdata.putAll(search);
                            cdata.put("withPages", "yes");// 是否分页
                            cdata.putAll(cindexKeyList);// 添加 cindex_key
                            modelAndView.addAllObjects(cdata);
    ...
    public ModelAndView getTitleCondition(@PathVariable(value = "titleId",required = true)String titleId, @PathVariable(value = "condition",required = true)String condition, HttpServletRequest request, HttpServletResponse response){
        ...
        if (isInteger(titleId)){
            ...
            if (condition.equals("content")){
                modelAndView = new ModelAndView("vm-productDetail");
                ...
                modelAndView = new ModelAndView("vm-productPrice");
                List<Map<String, Object>> everyOneSerach = getEveryOneSerachFromRedis(titleId);
                List<Map<String, Object>> brandZoneList = getBrandZoneListFromRedis(titleId);
                List<Map<String, Object>> zhuanTiGoodsImages = getZhuanTiGoodsImagesFromRedis(titleId);
                List<Map<String, Object>> tecentList = getTecentList(Integer.valueOf(titleId));
                ...
                modelAndView = new ModelAndView("vm-productPicture");
                List<Map<String, Object>> everyOneSerach = getEveryOneSerachFromRedis(titleId);
                List<Map<String, Object>> brandZoneList = getBrandZoneListFromRedis(titleId);
                List<Map<String, Object>> zhuanTiGoodsImages = getZhuanTiGoodsImagesFromRedis(titleId);
...
//                List<Map<String, Object>> brandZoneList = getBrandZoneList(Integer.valueOf(titleId));
                ...
                modelAndView = new ModelAndView("vm-productComments");
                List<Map<String, Object>> everyOneSerach = getEveryOneSerachFromRedis(titleId);
                List<Map<String, Object>> brandZoneList = getBrandZoneListFromRedis(titleId);
                List<Map<String, Object>> zhuanTiGoodsImages = getZhuanTiGoodsImagesFromRedis(titleId);
                List<Map<String, Object>> productComments = null;
                ...
                modelAndView = new ModelAndView("vm-productConsult");
                List<Map<String, Object>> everyOneSerach = getEveryOneSerachFromRedis(titleId);
                List<Map<String, Object>> brandZoneList = getBrandZoneListFromRedis(titleId);
                List<Map<String, Object>> zhuanTiGoodsImages = getZhuanTiGoodsImagesFromRedis(titleId);
                List<Map<String, Object>> productAdvisory = null;
                ...
                modelAndView = new ModelAndView("vm-productRelation");
                List<Map<String, Object>> everyOneSerach = getEveryOneSerachFromRedis(titleId);
                List<Map<String, Object>> brandZoneList = getBrandZoneListFromRedis(titleId);
                List<Map<String, Object>> zhuanTiGoodsImages = getZhuanTiGoodsImagesFromRedis(titleId);
                List<Map<String, Object>> productRelation = getProductRelationFromRedis(titleId);
    ...
    private List<Map<String,Object>> getEveryOneSerach(Integer titleId){
        ...
        try{
            ...
            Map<String, String> categoryInfoMap = prodDetailService.getProductMultiProperties(tgoodId, properties);
            logger.info("调用dubbo接口prodDetailService.getProductMultiProperties返回数据:"+com.alibaba.fastjson.JSONObject.toJSONString(categoryInfoMap));
            if (categoryInfoMap != null) {
                String skuId = String.valueOf(categoryInfoMap.get("skuItemIds"));
                if (skuId != null && !"".equals(skuId)) {
                    String[] skuIds = skuId.split(":");
    ...
    private List<Map<String,Object>> getEveryOneSerachFromRedis(String titleId){
        ...
        if (cache.exists("seotopic_everyOneSerach/"+titleId)){
            try {
                String everyOneSerachString = cache.get("seotopic_everyOneSerach/"+titleId);
                ...
                if (org.apache.commons.lang3.StringUtils.isNotBlank(everyOneSerachString)){
                    cache.set("seotopic_everyOneSerach/"+titleId,everyOneSerachString);
                }
            }
        }else {
            everyOneSerach = getEveryOneSerach(Integer.valueOf(titleId));
    ...
    private List<Map<String,Object>> getBrandZoneListFromRedis(String titleId){
        ...
        if (cache.exists("seotopic_brandZoneList/"+titleId)){
            try {
                String brandZoneListString = cache.get("seotopic_brandZoneList/"+titleId);
                ...
                if (org.apache.commons.lang3.StringUtils.isNotBlank(brandZoneListString)){
                    cache.set("seotopic_brandZoneList/"+titleId,brandZoneListString);
                }
            }
        }else {
            brandZoneList = getBrandZoneList(Integer.valueOf(titleId));
